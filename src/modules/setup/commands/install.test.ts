...
class MockTool implements ToolModule {
	...
	constructor(
		...
		displayName: string,
		installed: boolean,
		installShouldSucceed: boolean = true,
	) {
		this.name = name;
...
describe("InstallCommand", () => {
	...
	test("should install specific tool (nvm) when tool name provided", async () => {
		...
		runner.setResponseWithArgs("uname", ["-r"], {
			stdout: "5.0.0",
			stderr: "",
			exitCode: 0,
		});
		runner.setResponseWithArgs("uname", ["-s"], {
			stdout: "Linux",
			stderr: "",
			exitCode: 0,
		});
		...
		expect(result.message).toContain(
			"Node Version Manager installed successfully",
		);
	});
