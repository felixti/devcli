import type { CheckResult, PlatformInfo, ServiceContainer } from "@/kernel/types";
import type { ToolModule, ToolRegistry } from "@/modules/setup/tools/registry";
import type { WslConfigRecommendation, WslConfigService } from "@/wsl/wslconfig.types";
...
export class DoctorCommand {
  ...
  async execute(options: DoctorOptions): Promise<number> {
    ...
    const toolsToCheck = options.tool ? [this.registry.get(options.tool)] : this.registry.getAll();
  ...
  private async displayWslConfigRecommendation(
    ...
    if (current) {
      ...
      formatter.warn(`Current .wslconfig: ${currentCpu} CPUs, ${currentMem} GB RAM`);
