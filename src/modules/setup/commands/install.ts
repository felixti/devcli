...
export class InstallCommand {
	...
	async execute(options: InstallCommandOptions): Promise<InstallCommandResult> {
		const { toolName, yes, force, runner, prompter, fileSystem, platform } =
			options;

		if (toolName) {
			return this.installSingleTool(
				toolName,
				yes,
				force,
				runner,
		...
		return this.installAllMissingTools(
			yes,
			force,
			runner,
			prompter,
	...
	private async installSingleTool(
		...
		const installResult = await tool.install(runner, prompter, platform);
