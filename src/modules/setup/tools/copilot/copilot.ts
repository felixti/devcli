...
export class CopilotTool implements ToolModule {
	...
	async check(
		...
		if (extensionCheck.exitCode === 0) {
			...
			const configured = extensionCheck.stdout.includes("Logged in");
		...
		const standaloneCheck = await runner.run("github-copilot-cli", [
			"--version",
		]);
	...
	async install(
		...
		if (ghCheck.exitCode !== 0) {
			...
			const ghInstalled = await this.installGh(runner, prompter, platform);
		...
		const installResult = await runner.run("gh", [
			"extension",
			"install",
			"github/gh-copilot",
		]);
		...
		return {
			...
			success: true,
			message:
				"GitHub Copilot extension installed successfully. Run 'gh auth login' to configure.",
		};
	}
	...
	private async installGh(
		...
		if (packageManager === "apt") {
			...
			const installResult = await runner.run("apt-get", [
				"install",
				"-y",
				"gh",
			]);
