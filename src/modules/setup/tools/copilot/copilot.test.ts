...
const createPlatform = (platform: Platform) => ({
  platform,
  shell: "bash",
  packageManager: platform === "macos" ? "brew" : platform === "windows" ? "winget" : "apt",
  isWSL: false,
});
...
test("check - gh extension path: installed and configured", async () => {
  ...
  const result = await tool.check(runner, createPlatform("linux").platform, fileSystem);
...
test("check - standalone github-copilot-cli path", async () => {
  ...
  const result = await tool.check(runner, createPlatform("macos").platform, fileSystem);
...
test("install - gh installed, installs extension successfully", async () => {
  ...
  const result = await tool.install(runner, mockPrompter, createPlatform("linux").platform);
...
test("install - gh not installed triggers gh installation", async () => {
  ...
  const result = await tool.install(runner, mockPrompter, createPlatform("macos").platform);
