...
export class NvmTool implements ToolModule {
  ...
  private getVersionCommand(platform: Platform): { command: string; args: string[] } {
  ...
  private getConfiguredCheckCommand(platform: Platform): { command: string; args: string[] } {
    if (this.isWindowsPlatform(platform)) {
  ...
  async check(runner: ProcessRunner, platform: Platform, _fileSystem: FileSystem): Promise<CheckResult> {
    ...
    let message = "nvm is installed but not configured with an active Node version";
    ...
    if (configResult.exitCode === 0) {
      ...
      if (platform === "windows") {
        ...
        if (output && !output.includes("No installations recognized") && output.length > 0) {
          configured = true;
  ...
  async install(
    ...
    const mockFs: FileSystem = { exists: async () => false, readFile: async () => "", writeFile: async () => {}, mkdirp: async () => {} };
    ...
    if (this.isWindowsPlatform(platform)) {
      ...
      installMessage = result.exitCode === 0
      ...
      const result = await runner.run("bash", ["-c", `curl -o- ${installScript} | bash`]);

      installSuccess = result.exitCode === 0;

      if (installSuccess) {
        installMessage = "nvm installed successfully via official install script";
  ...
  private async updateShellProfile(
    ...
    const profileCheck = await runner.run("cat", [profilePath]).catch(() => ({ stdout: "", exitCode: 1 }));
