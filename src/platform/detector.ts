import type { PlatformInfo, DetectorDeps } from './types';

async function isWSL(env: Record<string, string>, readFile: (path: string) => Promise<string>): Promise<{ isWSL: boolean; wslVersion?: 'wsl1' | 'wsl2' }> {
...
export async function detectPlatform(deps: DetectorDeps): Promise<PlatformInfo> {
  const { env, readFile } = deps;

  const osPlatform = env['OS'] === 'Windows_NT' ? 'win32' : env['Platform'] || process.platform;
